...
export class ImeDetector {
  ...
  async start(): Promise<void> {
    if (process.platform === 'darwin') {
      const started = await this.startNativeWatcher();
      if (!started) {
  ...
  // --- macOS Native Watcher ---

  private async startNativeWatcher(): Promise<boolean> {
    try {
  ...
  private parseMacOSDefaultsOutput(output: string): void {
    // Try "Input Mode" first (for input methods like Korean, Chinese, Japanese)
    const inputModeMatch = output.match(/"Input Mode"\s*=\s*"([^"]+)"/);
    if (inputModeMatch) {
      this.notify(inputModeMatch[1]);
      return;
    }

    // Try "KeyboardLayout Name" (for keyboard layouts like US, German, French)
    const layoutMatch = output.match(/"KeyboardLayout Name"\s*=\s*"?([^";}\n]+)/);
    if (layoutMatch) {
      const name = layoutMatch[1].trim();
      this.notify(`com.apple.keylayout.${name}`);
      return;
    }

    // Try "Bundle ID" as last resort
    const bundleMatch = output.match(/"Bundle ID"\s*=\s*"([^"]+)"/);
    if (bundleMatch) {
      this.notify(bundleMatch[1]);
    }
  }
